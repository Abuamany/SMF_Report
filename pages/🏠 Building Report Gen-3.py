...
def get_conversational_chain():
    ...
    **Jawaban Utama**: Sebagai ahli LLM AI, jawaban utama melibatkan menyediakan strategi komprehensif, metodologi, atau kerangka logis. Ini harus menjelaskan pemikiran di balik jawaban dan memecah konsep-konsep kompleks menjadi langkah-langkah yang dapat dimengerti. Ini akan mencakup menyoroti fitur atau aspek kunci yang terkait dengan topik dan memberikan informasi tambahan untuk memperkaya pemahaman pengguna.
...
def main():
    ...
    st.title("Virtual Assistant Untuk membuat dan Meringkas Laporan Assessmen secara terpadu🪄")
...
def save_chat_to_pdf(messages, output_directory, file_name):
    # Membuat nama file PDF dengan waktu sekarang
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_file = os.path.join(output_directory, f"{file_name}_{timestamp}.pdf")
    ...
    for message in messages:
        role = message["role"]
        content = message["content"]  # Tidak perlu decode string
    ...
    # Membuka file PDF untuk penulisan
...
if __name__ == "__main__":
    main()
    file_name = ""
    file_name = st.sidebar.text_input("Nama file PDF:")
    if file_name.strip() == "":
        st.sidebar.error("Mohon masukkan nama file")
